...
struct ActiveTrackingCardInstanceTests {
    ...
    @Test func decode_legacyJson_migratesSuccessfully() {
        // speedCurve is hero-only; vertical and distance are grid-only; "profile" is dropped (unknown kind)
    ...
    @Test func registry_allHeroKinds_noGaps() {
        ...
        let expectedKinds: [ActiveTrackingCardKind] = [.speedCurve, .altitudeCurve, .heartRateCurve]
